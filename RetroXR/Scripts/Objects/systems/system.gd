...
func can_capture_state() -> Dictionary:
	...
	if _cores_without_states.has(_resolve_core()):
...
func capture_state(into_id := "") -> void:
	...
	if not bool(gate["ok"]):
		state_captured.emit(into_id, false, str(gate["reason"]))
		return

	_capture_id = into_id if not into_id.is_empty() else StatePaths.mint_id()
	_capture_core = _resolve_core()
	_capture_rom = rom_path
...
func _finish_capture(ok: bool, reason: String) -> void:
	...
	state_captured.emit(id, ok, reason)
...
func load_state(state_id: String) -> void:
	if not _load_id.is_empty():
		state_loaded.emit(state_id, false, "a save state is already loading")
		return
	var core := _resolve_core()
	if core.is_empty() or rom_path.is_empty():
		state_loaded.emit(state_id, false, "no game is inserted")
		return
	if not FileAccess.file_exists(StatePaths.state_path(core, rom_path, state_id)):
		state_loaded.emit(state_id, false, "that save state is missing")
		return
	_load_id = state_id
	_load_gen += 1
	# Reading it is the slow half on a big state, so it is not done here.
	WorkerThreadPool.add_task(_read_state_task.bind(core, rom_path, state_id, _load_gen))
...
func _issue_load(data: PackedByteArray, frame: int, gen: int) -> void:
	...
	if data.is_empty():
		...
		return
	_libretro.savestate_loaded.connect(_on_savestate_loaded, CONNECT_ONE_SHOT)
	get_tree().create_timer(STATE_ANSWER_TIMEOUT).timeout.connect(
		func() -> void:
			if _load_gen == gen and not _load_id.is_empty():
				if _libretro.savestate_loaded.is_connected(_on_savestate_loaded):
					_libretro.savestate_loaded.disconnect(_on_savestate_loaded)
	...
	_libretro.RequestLoadState(data, frame)
...
func _finish_load(ok: bool, reason: String) -> void:
	...
	_load_id = ""
	_load_gen += 1
	state_loaded.emit(id, ok, reason)
