...
## Build the spatialised audio emitter fed by VlcPlayer's PCM ring buffer. As a
## child of the (pickable) unit it follows the device around automatically, and
## the shared emitter adds occlusion muffling + distance filtering + doppler.
func _setup_audio() -> void:
	_emitter = SpatialAudioEmitter.new()
	add_child(_emitter)
	_emitter.configure(float(_vlc.get_audio_rate()) if _vlc else 48000.0, SpatialAudioPresets.TABLETOP)
...
func _apply_volume() -> void:
	if _emitter:
		_emitter.set_volume_db(-80.0 if _scan_dir != 0 else _volume_db())
